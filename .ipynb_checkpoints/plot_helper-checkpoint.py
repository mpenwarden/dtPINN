...
# Plot reference solution
def result_plot(userPar, dataPar, model, iterations, l2_error_list, epoch_list, epoch_record_list, fig_num):
    
    return_residual = 1;
    fontsize = 20; fontsize_legend = 12.5;
    ...
    if userPar['save_fig'] == 1:
        plt.savefig(str(Path.cwd()) + '\\figures\\' + userPar['model_name'] + '_fig1_' + str(fig_num))
    ...
    if userPar['save_fig'] == 1:
        plt.savefig(str(Path.cwd()) + '\\figures\\' + userPar['model_name'] + '_fig2_' + str(fig_num))
    if userPar['show_fig'] == 1:
        plt.show()
    plt.close()
    ...
    fig_num += 1
    return l2_error_list, epoch_list, epoch_record_list, fig_num