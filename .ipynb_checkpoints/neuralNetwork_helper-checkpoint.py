...
class dtPINN:
    def __init__(self, dataPar, userPar):
        ...
        self.tmin = dataPar['tmin']; self.tmax = dataPar['tmax']; self.xmin = dataPar['xmin']; self.xmax = dataPar['xmax'];
        
        # Initalize window-sweeping
        self.window_scheme = userPar['window_scheme']
        if self.window_scheme != 'none':
            self.window_sweep = window_sweep_class(userPar['window_scheme'], userPar['scheme_parameters'], userPar['tmin'], userPar['tmax'])
            self.propogate_loss_tol = scheme_parameters[1]
            self.propogate_dt = scheme_parameters[2]
    ...
    def get_loss(self):
        ...
        for i in range(self.num_partitions - self.dS, self.num_partitions):
            ...
            #if i != self.num_partitions-1: # Include final interface or not
            xi = self.xi_decomp[i]; ti = self.ti_decomp[i];
            ...
            if self.window_scheme != 'none':
                ...
                b_weights = self.window_sweep.get_weights(tb_max.detach().numpy())
                ...
                f_weights = self.window_sweep.get_weights(tf.detach().numpy())           
            ...
            if self.input_encoding == 0:
                ...
                if self.window_scheme != 'none':
                    mse_ub += ((ub_max - ub_min)*b_weights).square().mean()
                else:
                ...
                for j in range(len(variables_b_max)):
                    if self.window_scheme != 'none':
                        mse_ub += ((variables_b_max[j] - variables_b_min[j])*b_weights).square().mean()
                    else:
        ...
        # Sum losses
        bound_loss = self.w_b()*(mse_ub)*100 # Boundary condition
        init_loss = self.w_b()*(mse_u0)*100 # Initial condition
        bc_loss = self.w_b()*(mse_bc)*100 # Backward-compadability
        res_loss = self.w_rc()*(mse_f) # Residual
        inter_loss = self.w_i()*(mse_i)*100 # Interface
    ...
    def check_window_sweep(self, window_vars):
        ...
        if abs(self.mse_f_prev - mse_f) < torch.tensor(self.propogate_loss_tol):
            ...
            self.window_sweep.propogate(self.propogate_dt)
    ...
    def train_lbfgs(self, optim_state, optim_state_dict, optimizer):
        ...
        def closure():
            global iter_count
            if self.new_partition == 0 and self.end_condition == 0:
                iter_count += 1
                optimizer.zero_grad()
                bound_loss, init_loss, bc_loss, res_loss, inter_loss, weight_loss, window_vars = self.get_loss()
                loss = bound_loss + init_loss + bc_loss + res_loss + inter_loss + weight_loss
        ...
        return optimizer.state_dict(), optimizer, iter_count, self.new_partition, self.end_condition
    ...
    def predict(self, u_decomp_test, t_decomp_test, x_decomp_test, return_residual):
        ...
        for i in range(self.num_partitions):
            dnet = self.u_dnet[i]
            a = self.a_list[i]
            t_test = t_decomp_test[i];  x_test = x_decomp_test[i]; u_test = u_decomp_test[i]
            if self.window_scheme != 'none':
            ...
            if return_residual == 1:
                if self.window_scheme != 'none':
                    ...
                    res_weights = self.window_sweep.get_weights(t_res.detach().numpy())
                    u_res = self.forward(dnet, t_res, x_res, a())
                    variables = self.get_partials(u_res, t_res, x_res, 0)
                    res = (self.PDE.PDE(variables)*res_weights).square()
